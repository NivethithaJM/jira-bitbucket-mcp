...
export const getPullRequestsForIssueToolDefinition: Tool = {
...
export async function getPullRequestsForIssue(issueKey: string) {
  try {
    ...
    const sortedPRs = allPullRequests.sort((a: any, b: any) => {
    ...
    const tableData = sortedPRs.map((pr: any) => ({
